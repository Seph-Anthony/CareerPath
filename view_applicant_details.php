...
// 4. Main Query: Fetch Applicant, Application (from intern_application), and Posting Details
// IMPORTANT: We remove the ip.company_id check from the WHERE clause temporarily for debugging/simplification.
// The ip.company_id is fetched below to check ownership manually.
$query = "
 SELECT 
 s.first_name, 
 s.last_name, 
...
ip.title AS post_title,
        ip.company_id AS posting_company_id, /* FETCH THIS FOR THE OWNERSHIP CHECK */
 ...
 WHERE
 ia.student_id = ? AND ia.posting_id = ? 
 LIMIT 1
";

$stmt = $mysqli->prepare($query);
if ($stmt) {
    // Bind only student_id and posting_id now
...
$current_status = htmlspecialchars($applicant_data['application_status']);
        ...
        // This 'else' catches the case where the application record simply doesn't exist
...
            <a href="manage_applicants.php" style="color: rgb(32, 64, 227); text-decoration: none; font-weight: 600;"><i class="fa-solid fa-arrow-left"></i> Back to Applicants List</a>
        </header>
...
                    <p>Applied for: **<?php echo $post_title; ?>**</p>
                </div>
...
                
                    <p>Current Status: 
                        <span class="status-tag status-<?php echo strtolower($current_status); ?>"><?php echo $current_status; ?></span>
                    </p>
                
